...
def add_book():
    ...
    if request.method == "POST":
        ...
        if len(book_title) < 3 or len(book_title) > 50:
            errors.append(
                'Book title must be between 3 to 50 characters long.')
        if len(book_title) < 3 or len(book_author) > 50:
            errors.append(
                'Book author must be between 3 to 50 characters long.')
        if len(book_genre) < 3 or len(book_title) > 20:
            errors.append(
                'Book genre must be between 3 to 20 characters long.')
        if len(errors) == 0:
            book = Book(
